...
interface StepPartnerB_WorkLifeProps {
  data: PartnerProfile;
...
export const StepPartnerB_WorkLife: React.FC<StepPartnerB_WorkLifeProps> = ({ data, setData, onNext, onPrevious }) => {
  ...
  return (
    <div className="space-y-5">
      <FormInput label={t('form_partner_b_work_occupation_label')} name="occupation" value={data.occupation} onChange={handleChange} placeholder={t('form_input_occupation_placeholder')} />
      
      <FormSelect 
        label={t('form_partner_b_work_hours_label')}
        name="workHours" 
        value={workHoursSelectValue} 
        onChange={handleChange} 
        options={workHoursOptions} 
        firstOption={{value: "", label: t('form_select_partner_work_hours_default')}}
      />
      {showOtherWorkHours && (
        <FormInput 
            label={t('form_partner_b_work_hours_other_label')} 
            name="workHoursCustomInput"
            value={data.workHours === "other_wh_custom" ? "" : data.workHours} 
            onChange={handleWorkHoursTextChange} 
            placeholder={t('form_input_partner_other_work_hours_placeholder')} 
        />
      )}

      <FormSelect 
        label={t('form_partner_b_work_stress_label')}
        name="stressLevel" 
        value={data.stressLevel} 
        onChange={handleChange} 
        options={stressLevelOptions} 
        firstOption={{value: "", label: t('form_select_partner_stress_default')}}
      />
      <FormSelect 
        label={t('form_partner_b_work_financial_label')} 
        name="financialSituation" 
        value={data.financialSituation} 
        onChange={handleChange} 
        options={financialSituationOptions} 
        firstOption={{value: "", label: t('form_select_partner_financial_default')}}
      />
